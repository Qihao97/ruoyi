    ...
    $.extend({
        ...
        operate: {
            ...
            setDevice: function(id) {
                $.modal.open("设置" + $.table._option.modalName, $.operate.setDeviceUrl(id));
            },
            // 通行设置
            setPass: function(id) {
                if($.common.isEmpty(id) && $.table._option.type ==
                    table_type.bootstrapTreeTable) {
                    var row = $('#' + $.table._option.id).bootstrapTreeTable
                    ('getSelections')[0];
                    if ($.common.isEmpty(row)) {
                        $.modal.alertWarning("请至少选择一条记录");
                        return;
                    }
                    var url = $.table._option.updateUrl.replace("{id}", row
                        [$.table._option.uniqueId]);
                    $.modal.open("修改" + $.table._option.modalName, url);
                } else {
                    $.modal.open("修改" + $.table._option.modalName, $.operate.setPassUrl
                    (id));
                }
            },
            // 考勤设置
            setAttendance: function(id) {
                if($.common.isEmpty(id) && $.table._option.type ==
                    ...
                    $.modal.open("修改" + $.table._option.modalName,
                        $.operate.setAttendanceUrl(id));
                }
            },
            // 设备设置访问地址
            setDeviceUrl: function(id) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = $.table._option.setDeviceUrl.replace("{id}", id);
                } else {
                    var id = $.common.isEmpty($.table._option.uniqueId) ?
                        $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
                    if (id.length == 0) {
                        $.modal.alertWarning("请至少选择一条记录");
                        return;
                    }
            ...
            // 通信设置访问地址
            setPassUrl: function(id) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = $.table._option.setPassUrl.replace("{id}", id);
                } else {
                    var id = $.common.isEmpty($.table._option.uniqueId) ?
                        $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
                    if (id.length == 0) {
            ...
            // 考勤设置访问地址
            setAttendanceUrl: function(id) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = $.table._option.setAttendanceUrl.replace("{id}", id);
                } else {
                    var id = $.common.isEmpty($.table._option.uniqueId) ?
                        $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
                    if (id.length == 0) {
